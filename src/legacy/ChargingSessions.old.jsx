...
const ChargingSessions = () => {
  ...
  const loadSessions = async () => {
    try {
      ...
      // Fetch dashboard data which includes active sessions
      const dashboardData = await staffAPI.getDashboardOverview();
      console.log("✅ Dashboard data:", dashboardData);
      
      // Extract sessions from dashboard connectors
      const activeSessions = [];
      
      if (dashboardData?.connectors) {
        dashboardData.connectors.forEach(connector => {
          if (connector.currentBooking) {
            activeSessions.push({
              id: connector.currentBooking.bookingId,
              bookingId: connector.currentBooking.bookingId,
              connectorId: connector.connectorId,
              connectorName: connector.connectorType,
              customerName: connector.currentBooking.customerName || 'N/A',
              vehiclePlate: connector.currentBooking.vehicleInfo || 'N/A',
              startTime: connector.currentBooking.startTime,
              endTime: connector.currentBooking.endTime,
              duration: connector.currentBooking.duration || 0,
              energyConsumed: connector.currentBooking.energyConsumed || 0,
              estimatedCost: connector.currentBooking.cost || 0,
              status: connector.status === 'charging' ? 'in_progress' : connector.status,
              paymentStatus: connector.currentBooking.paymentStatus || 'pending',
            });
          }
        });
      }
      ...
      setSnackbar({
        open: true,
        message: error.message || "Không thể tải dữ liệu phiên sạc",
        severity: "error",
      });
